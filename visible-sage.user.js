...
// OneeChan themes don't have the "useremail" class anymore, so saged names are the same color
// If OneeChan is detected, we change the .nameBlock class to prevent the default color
var oneeChanDetected = false;
...
showSage = function(post) {
  ...
  for (var i=0; i<len; ++i) {
    ...
    var nameBlock = $.qsa("#pi"+post.id.substr(1)+" .nameBlock")[0];
    if (oneeChanDetected)
      nameBlock.className = "sagedNameBlock";
    mail.appendChild(nameBlock);
